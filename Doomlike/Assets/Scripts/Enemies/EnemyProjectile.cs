...
namespace Doomlike.Enemies
{
    public class EnemyProjectile : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                if (hit.collider.transform.IsChildOf(source != null ? source.transform : transform))
                {
                    // ignore self
                }
                else
                {
                    var ph = hit.collider.GetComponentInParent<Doomlike.Player.PlayerHealth>();
                    if (ph != null)
                    {
                        ph.ApplyDamage(new DamageInfo(damage, DamageType.Bullet, source,
